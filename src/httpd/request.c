			     ...
			     GList * arguments)
{
  ...
  for (list = arguments; list; list = list->next)
	...
	offset = atoi (kv->value);
      ...
      else if (!g_strcmp0 (kv->key, REQUEST_GET_ALL_DOCS_KEYS))
        {
  	  JsonNode * node = NULL;

          if ((!(parser = json_parser_new ())) ||
              (!json_parser_load_from_data (parser, (gchar *)kv->value, strlen (kv->value), &error)) ||
  	      (!(node = json_parser_get_root (parser))) ||
	      (json_node_get_node_type (node) != JSON_NODE_ARRAY) ||
	      (!(keys = json_array_get_elements (json_node_get_array (node)))))
            {
	      if (startkey != NULL)
                g_free (startkey);
              ...
              request_set_error (client, "Invalid " REQUEST_GET_ALL_DOCS_KEYS " parameter. Value must be an JSON array of keys.");
  ...
  if (keys != NULL
      ...
      || types != NULL
      || filter_by != NULL)
      total_rows = dupin_record_get_list_total (db, 0, 0, keys, startkey, endkey, inclusive_end, DP_COUNT_EXIST, types, types_op, 
  ...
  if (dupin_record_get_list (db, count, offset, 0, 0, keys, startkey, endkey, inclusive_end, DP_COUNT_EXIST,
...
//g_message("request_global_get_all_docs_linkbase: k=%s v=%s\n", (gchar *)kv->key, (gchar *)kv->value);
	...
	offset = atoi (kv->value);
      ...
      else if (!g_strcmp0 (kv->key, REQUEST_GET_ALL_LINKS_KEYS))
        {
          JsonNode * node = NULL;
          ...
          if ((!(parser = json_parser_new ())) ||
              ...
              (!(keys = json_array_get_elements (json_node_get_array (node)))))
            {
              ...
              request_set_error (client, "Invalid " REQUEST_GET_ALL_LINKS_KEYS " parameter. Value must be an JSON array of keys.");
				  ...
				  GList * arguments)
{
  ...
  if (dupin_view_record_get_list_total (view, &total_rows, 0, 0, keys, startkey, endkey, inclusive_end, 
  ...
  if (dupin_view_record_get_list (view, count, offset, 0, 0, DP_ORDERBY_KEY, descending,
				  keys, startkey, endkey, inclusive_end,
				  startvalue, endvalue, inclusive_end_value,
				  filter_by, filter_by_format, filter_op, filter_values,
				  &results, NULL) == FALSE)
    {
...
static DSHttpStatusCode request_global_post_all_docs_view (DSHttpdClient * client,
						           GList * path,
						           GList * arguments);
		     ...
		     GList * arguments)
{
  ...
  if (!g_strcmp0 (path->data, REQUEST_VIEWS))
    {
	  ...
	  if (!g_strcmp0 (path->next->next->data, REQUEST_POST_COMPACT_VIEW))
          ...
          /* POST /_views/view/_all_docs */
          if (!g_strcmp0 (path->next->next->data, REQUEST_POST_ALL_DOCS))
            return request_global_post_all_docs_view (client, path, arguments);
        }

      request_set_error (client, "POST /_views allowed commands are: /_views/view/_all_docs and /_views/view/_compact");
  ...
  if (!path->next->next && !g_strcmp0 (path->next->data, REQUEST_POST_ALL_DOCS))
    return request_global_post_all_docs (client, path, arguments);
  ...
  request_set_error (client, "POST /database allowed commands are: /database, /database/_bulk_docs, /database/_all_docs, /database/_compact, /database/doc_id, /database/doc_id/_bulk_links, /database/doc_id/_links, /database/doc_id/_relationships");
			      ...
			      GList * arguments)
{
  ...
  if (json_object_has_member (json_node_get_object (node), REQUEST_POST_ALL_DOCS_KEYS) == FALSE)
    {
      request_set_error (client, "Body does not contain a mandatory " REQUEST_POST_ALL_DOCS_KEYS " object member");
      code = HTTP_STATUS_500;
      goto request_global_post_all_docs_end;
    }

  JsonNode * keys_node = json_object_get_member (json_node_get_object (node), REQUEST_POST_ALL_DOCS_KEYS);

  if (json_node_get_node_type (keys_node) != JSON_NODE_ARRAY)
    {
      request_set_error (client, "Body object member " REQUEST_POST_ALL_DOCS_KEYS " is not an array");
      code = HTTP_STATUS_500;
      goto request_global_post_all_docs_end;
    }
  ...
  client->request_arguments = g_list_prepend (client->request_arguments,
                                dp_keyvalue_new (REQUEST_GET_ALL_DOCS_KEYS, json_keys));
			       ...
			       GList * arguments)
{
  ...
  client->request_arguments = g_list_prepend (client->request_arguments,
                                dp_keyvalue_new (REQUEST_GET_ALL_LINKS_KEYS, json_keys));
...
static DSHttpStatusCode
request_global_post_all_docs_view (DSHttpdClient * client,
			           GList * path,
			           GList * arguments)
{
  DSHttpStatusCode code;
...
//g_message("request_get_changes_comet_database: param_since=%" G_GSIZE_FORMAT " results_count=%d\n", client->output.changes_comet.param_since, (gint)g_list_length (results));
...
//g_message("request_get_changes_comet_database: record_id=%s record_mvcc=%s\n", record_id, record_mvcc);
		...
		if ((client->output.changes_comet.param_feed == DP_CHANGES_FEED_LONGPOLL) && (list->next != NULL))
		  g_string_append (str, ",");
...
//g_message("request_get_changes_comet_linkbase: param_since=%" G_GSIZE_FORMAT " results_count=%d\n", client->output.changes_comet.param_since, (gint)g_list_length (results));
...
//g_message ("RELS: %s\n", tmp1);
  ...
  if (types != NULL
      || filter_by != NULL)
      total_rows = dupin_record_get_list_total (db, 0, 0, NULL, NULL, NULL, TRUE, DP_COUNT_EXIST, types, types_op, 
						filter_by, filter_by_format, filter_op, filter_values, NULL);
  else
      total_rows = dupin_database_count (db, DP_COUNT_EXIST);
  ...
  if (dupin_record_get_list (db, count, offset, 0, 0, NULL, NULL, NULL, TRUE, DP_COUNT_EXIST,
				DP_ORDERBY_ID, descending, types, types_op,
				filter_by, filter_by_format, filter_op, filter_values, &results, NULL) == FALSE)
    {
      dupin_database_unref (db);
